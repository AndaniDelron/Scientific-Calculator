...
// Define the precise structure of the math object we expect from the CDN.
// This allows TypeScript to check if we use 'math.evaluate' correctly.
interface MathJsInstance {
    evaluate: (expression: string) => any;
    format: (value: any, options?: { precision: number }) => string;
}

// Tell TypeScript that the global 'math' variable exists and has the defined structure.
// We use a safe library (math.js) instead of the dangerous eval() function.
...
const processInput = (btnText: string, dataVal: string | null): void => {
    ...
    // C. EQUALS BUTTON ('=') - Calculation
        ...
        if (currentInput === '') return; // Don't calculate if nothing is entered
